...
// and undici@8 crashes at import time on Node < 24
// ("webidl.util.markAsUncloneable is not a function"). A version check
...
import { MIN_NODE_MAJOR, nodeMajorMeetsMinimum, oldNodeMessage } from "./lib/node-version.js";

if (!nodeMajorMeetsMinimum(process.versions.node, MIN_NODE_MAJOR)) {
  console.error(oldNodeMessage(process.versions.node, MIN_NODE_MAJOR));
  process.exit(1);
}
