// The agent runtime: config load, collection loop, alert evaluation, push.
//
// This is a separate module from index.ts on purpose. index.ts is the CLI
// front: it parses argv and handles --help / --version / init / enroll /
// doctor / mark-reboot, and only THEN dynamically imports this file for the
// run mode. ESM hoists static imports above every statement in a module, so
// while this code lived in index.ts its import graph (collectors, undici,
// alerts/state.js with its import-time read of the state file) was evaluated
// before the help check could run. On 2026-09-04 an unprivileged
// `glassmkr-crucible --help` printed two EACCES stack traces from the
// root-owned alert-state file ahead of the usage text. Nothing here may be
// statically imported from index.ts.
...
// index.ts has already parsed argv and established mode === "run" before
// importing this module; re-deriving the flags here keeps the front and the
// runtime decoupled without threading them through the dynamic import.
const { result: cliArgs } = parseCliArgs(process.argv.slice(2), PKG_VERSION);
